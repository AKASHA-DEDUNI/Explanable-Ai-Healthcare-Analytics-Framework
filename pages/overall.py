...
def sweep_thresholds(heatmap, gt_bboxes, image):
    top_percentile = np.percentile(heatmap, 90)
...
def evaluate_dataset_thresholds(dataset_folder, df_bbox, model, text_model, gradcam):
    ...
    for img_file in image_files:
        ...
        meta = torch.tensor([[30, 0]], dtype=torch.float32).to(DEVICE)
        ...
        for t in thresholds:
            ...
            if gt_bboxes:
                for gt_box in gt_bboxes:
                    ...
                    iou_scores.append(best_iou)
            avg_iou = np.mean(iou_scores) if iou_scores else 0
            all_results[round(t, 2)].append(avg_iou)

    mean_ious = {t: np.mean(scores) if scores else 0 for t, scores in all_results.items()}
    best_threshold = max(mean_ious, key=lambda x: mean_ious[x])
    return mean_ious, best_threshold
...
if st.button("Evaluate Dataset Thresholds"):
    if not os.path.exists(dataset_folder):
        ...
        mean_ious, best_t = evaluate_dataset_thresholds(dataset_folder, df_bbox, model, text_model, gradcam)

        df = pd.DataFrame(list(mean_ious.items()), columns=["Threshold", "Mean IoU"])
        st.dataframe(df)

        # Plot chart
        fig, ax = plt.subplots()
        ax.plot(df["Threshold"], df["Mean IoU"], marker="o")
        ax.set_xlabel("Threshold")
        ax.set_ylabel("Mean IoU")
        ax.set_title("Threshold vs Mean IoU")
        st.pyplot(fig)

        st.success(f"✅ Global Best Threshold: {best_t} | Mean IoU: {mean_ious[best_t]:.3f}")
