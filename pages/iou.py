...
def union_boxes(boxes):
    """Compute bounding rectangle that covers all boxes"""
    if not boxes:
        return None
...
# --------------------- Streamlit App ---------------------
st.title("📊 Pneumonia Grad-CAM Bounding Box & IoU")

# Initialize session state
if "all_ious" not in st.session_state:
    st.session_state["all_ious"] = []
if "all_gt_boxes" not in st.session_state:
    st.session_state["all_gt_boxes"] = []
if "all_cam_boxes" not in st.session_state:
    st.session_state["all_cam_boxes"] = []
...
if st.button("Run Prediction & Compute IoU"):
    if uploaded_file is None or report.strip() == "":
        ...
        st.session_state["all_ious"].extend(iou_scores)
        st.session_state["all_gt_boxes"].extend(gt_bboxes)
        st.session_state["all_cam_boxes"].extend(cam_bboxes)
...
if "results" in st.session_state:
    ...
    # --- Per-image IoU ---
    if res["iou_scores"]:
        st.subheader("Per-image IoU Scores")
...
# --- Overall Mean IoU ---
if st.session_state["all_ious"]:
    mean_iou = np.mean(st.session_state["all_ious"])
    st.subheader("📊 Overall Mean IoU Across Dataset")
    st.metric(label="Mean IoU", value=f"{mean_iou:.3f}")

# --- Dataset-level IoU ---
dataset_gt_union = union_boxes(st.session_state["all_gt_boxes"])
dataset_cam_union = union_boxes(st.session_state["all_cam_boxes"])
if dataset_gt_union and dataset_cam_union:
    dataset_level_iou = compute_iou_boxes(dataset_gt_union, dataset_cam_union)
    st.subheader("📊 Dataset-level IoU (Union of Boxes)")
    st.metric(label="Dataset IoU", value=f"{dataset_level_iou:.3f}")
