...
def explain_gradcam(pred_class, confidence, iou_scores, num_cam_regions, user_type="Doctor"):
    if not GROQ_API_KEY:
        return "❌ GROQ_API_KEY not set. Please configure your .env file."

    if user_type == "Doctor":
        style_instruction = """
        Write the explanation for a radiologist or doctor. 
        Use precise medical terminology, reference lung zones, radiological patterns 
        (e.g., consolidation, infiltrates, ground-glass opacities), and provide a 
        technical interpretation of Grad-CAM heatmaps. 
        """
    else:
        style_instruction = """
        Write the explanation for a patient. 
        Avoid technical jargon, explain in simple language, 
        and focus on what the highlighted regions mean for their health. 
...
user_type = st.radio("Who is the explanation for?", ["Doctor", "Patient"])
